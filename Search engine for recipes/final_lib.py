...
#=========================
def create_index(ListaDocs):
    inverted_index = {}
    i=0
    for k, v in ListaDocs.items():
    ...
    if Veg:
        ...
        for i in range(len(ListIntersQuery)):
            if MatOrig.loc[ListIntersQuery[i]][2]=='Vegetarian':
                count+=1
                for j in query:
                    if j in MatOrig.loc[ListIntersQuery[i]][0]:# title
                       cos[0][i]*= 10
                    if j in MatOrig.loc[ListIntersQuery[i]][6]:# ingredients
                       cos[0][i]*= 5
            else:         
               cos[0][i]*= 0
        return cos,count
...
def ranklist(inv,listwords,matrix,MatOrig) :
    ...
    print('There are',len(rank_q),'recipes')
    return rank_q
